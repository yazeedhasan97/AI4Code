...
class BModel(nn.Module):
    ...
    def define_cnn_layers(self):
        ...
        self.top1 = nn.Linear(classes, output_length) 
        self.top2 = nn.Linear(output_length, 1)
    
    
    def forward(self, ids, mask):
        dbert = self.bert_cnn_forward(ids, mask)
        return dbert
    
    def bert_cnn_forward(self, ids, masks):
        ...
        x = torch.nn.functional.leaky_relu(self.top1(x), 0.00001)
...
class BDataset(Dataset):
    def __init__(self, df, source_dict, bert_model_name, max_len, total_max_len, ):
        super().__init__()
        self.df = df
    ...
    def __getitem__(self, index):
        ...
        txt = self.source_dict[row[0]] + '[SEP]' + self.source_dict[row[1]]
        ...
        return torch.LongTensor(ids), torch.LongTensor(mask), torch.FloatTensor([label])

    def __len__(self):
        return len(self.df)

    
def generate_testing_triplet(df, size=1000, drop=0.9):
    triplets = []
    ids = df.id.unique()
...
def generate_training_triplet(df, mode='train', size=1000, drop=0.9):
    triplets = []
    ids = df.id.unique()
    random_drop = np.random.random(size=size) > drop
    count = 0
...
def validate(model, val_loader):
    ...
    with torch.no_grad():
        for idx, data in enumerate(tbar):
            ...
            preds[count:count+len(pred)] = pred.detach().cpu().numpy().ravel()
            count += len(pred)
            labels.append(target.detach().cpu().numpy().ravel())
    
    return np.concatenate(labels), preds

def train(model, train_loader, val_loader, epochs, opt='nadam', model_name='distil', patience=2, path=None):
    np.random.seed(42)
    ...
    for e in range(epochs):   
        ...
        for idx, data in enumerate(tbar):
            ...
            tbar.set_description(f"Epoch {e+1} Loss: {np.round(np.mean(loss_list), 7)} lr: {np.round(optimizer.param_groups[0]['lr'], 5)}")

        y_val, y_pred = validate(model, val_loader)
        
        print("Validation MAE:", np.round(mean_absolute_error(y_val, y_pred), 7))
        print()
